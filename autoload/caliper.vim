...
augroup CaliperGroup
  autocmd!
  autocmd WinEnter * let w:caliper_cache = {}
augroup END

func! caliper#horizontal(n)
  execute printf('resize %+d', a:n * s:fetch('s:horizontal_sign'))
endf

func! caliper#vertical(n)
...
func! s:fetch(funcname)
  let cached_val = get(w:caliper_cache, a:funcname)
  if cached_val
    return cached_val
  else
    let w:caliper_cache[a:funcname] = call(a:funcname, [])
    return w:caliper_cache[a:funcname]
  endif
endf
